#' GDT
#' to be documented
#' @usage GDT(j, Y, X, S, H, kernels, minv = 0, maxknn = 500, NmaxDist = 6000,isgcv, TIME, decay)
...
function (j, Y, X, S, H, kernels, minv = 0, maxknn = 500, NmaxDist = 6000,
    isgcv, TIME, decay)
{
    w <- kernel_C(as.matrix(S[, 1:2]), j, H[1], kernels[1], minv,
        FALSE, decay, isgcv, TRUE) * kernel_C(as.matrix(S[, 3]),
        j, H[2], kernels[2], minv, TRUE, decay, isgcv, TRUE)
    if (sum(w > 0) < minv)
        kernel_C(as.matrix(S[, 1:2]), j, H[1], kernels[1], minv,
            FALSE, decay, isgcv, TRUE) * kernel_C(as.matrix(S[,
            3]), j, H[2], kernels[2], minv, FALSE, decay, isgcv,
            TRUE)
    else w
}
