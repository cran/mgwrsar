...
if(FALSE){
W_extra=kernel_matW(S=rbind(model$S,newdata_coords),H=mykernels_H,diagnull=FALSE,kernels=mykernels_extra,adaptive=mykernels_adaptive,NN=k_extra[1],Type=model$Type,query_TP=1:nrow(model$S),rowNorm=TRUE,correctionNB=FALSE,extrapTP=1)
...
S=1:800
O=801:1000
W_extra=kernel_matW(S=coords,H=100,diagnull=FALSE,kernels="sheppard",adaptive=FALSE,NN=16,Type='GD',query_TP=S,rowNorm=TRUE,correctionNB=FALSE,extrapTP=0)
W_extra=W_extra[O,S]
}
