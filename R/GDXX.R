#' GDXX
#' to be documented
#' @usage GDXX(j, Y, X, S, H, kernels, minv = 0, maxknn = 500, NmaxDist = 6000,isgcv, TIME, decay)
...
kernel_C(as.matrix(S[, 1:2]), j, H[1], kernels[1], minv, FALSE, decay, isgcv, TRUE)*kernel_C(as.matrix(S[, 3]), j, H[2], kernels[2], minv, FALSE, decay, isgcv, TRUE)*kernel_C(as.matrix(S[, 4]), j, H[3], kernels[3], minv, FALSE, decay, isgcv, TRUE)
}
