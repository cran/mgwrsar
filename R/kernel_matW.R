...
#' }
kernel_matW <- function(H, kernels, coords, NN = NULL, TP = NULL, Type = 'GD',
                        adaptive = FALSE, diagnull = TRUE, alpha = 1,
                        dists = NULL, indexG = NULL, extrapol = FALSE, QP = NULL, K = 0) {
  n <- nrow(coords)

  if (is.null(NN)) {
    if (kernels[1] == 'gauss') NN = n else NN = min(n, H[1] + 2)
  } else {
    if (adaptive[1]) NN = max(NN, H[1] + 2)
  }

  if (Type == 'GDT') {
    if (kernels[1] != 'gauss' | unlist(str_split(kernels[2], '_'))[1] != 'gauss')
      stop('For Type=GDT only gauss kernel should be used')
  }
  ...
  if ((ncol(coords) > 2 | Type != 'GD') & (length(kernels) < ncol(coords) - 2 | length(H) < ncol(coords) - 2))
    stop("if Type!='GD', H and kernels must be indicated for each variables used in General Kernel Product")

  if ((ncol(coords) > 2 | Type != 'GD') & (length(adaptive) == 1))
    if (adaptive) stop("if Type!='GD',adaptive must be indicated for each variables used in General Kernel Product")

  if (is.null(indexG)) {
    stage0 = prep_d(coords, NN = NN, TP = TP, extrapol = extrapol, ratio = 1, QP = QP, kernels = kernels, Type = Type)
